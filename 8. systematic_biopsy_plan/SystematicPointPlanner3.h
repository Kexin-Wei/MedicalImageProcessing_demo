...
private:
    ...
    const std::map<int, std::vector<QVector3D>> getModelLineIntersectionPoints(vtkSmartPointer<vtkPolyData> modelPolyData, double* bounds, std::vector<double> lines);
    const std::vector<double> getInterpolationLines(double minValue, double maxValue, std::vector<double> ratios);
