...
from lib.folder.med import MedicalImageFolderMgBase
from lib.med_image.accuracy_verify import (
    calc_dice_similarity_coefficient,
...
label_FMG = MedicalImageFolderMgBase(label_path)
prediction_FMG = MedicalImageFolderMgBase(prediction_path)
correction_FMG = MedicalImageFolderMgBase(correction_path)
...
def assess_predict(assess_images, save_path: Path):
    df = pd.DataFrame(
        columns=["Image Name", "DSC (%)", "HD (mm)", "RVD (%)", "Accuracy (%)"]
    )
    ...
    for l, p in zip(label_images, assess_images):
        ...
        assert (
            l_array.shape == p_array.shape
        ), f"label shape:{l_array.shape}, prediction shape:{p_array.shape}"
        assert (
            l_array.max() == 1 == p_array.max()
        ), f"label max:{l_array.max()}, prediction max:{p_array.max()}"
        assert (
            l_array.min() == 0 == p_array.min()
        ), f"label min:{l_array.min()}, prediction min:{p_array.min()}"
